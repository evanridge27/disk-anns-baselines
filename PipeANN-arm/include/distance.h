...
namespace diskann {
  ...
  // FP16 distance classes - convert fp16 to fp32 via SIMD, then compute in fp32
  class DistanceL2FP16 : public Distance<fp16_defined> {
   public:
    virtual float compare(const fp16_defined *a, const fp16_defined *b, uint32_t size) const __attribute__((hot));
  };

  class DistanceCosineFP16 : public Distance<fp16_defined> {
   public:
    virtual float compare(const fp16_defined *a, const fp16_defined *b, uint32_t length) const;
  };
