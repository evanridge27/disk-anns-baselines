...
namespace diskann {
  ...
  // FP16 conversion helpers - convert between T and float correctly
  // For fp16_defined (uint16_t), uses fp16_ieee_to_fp32_value / fp16_ieee_from_fp32_value
  // For other types, uses simple C-style cast
  template<typename T>
  inline float T_to_float(T val) {
    return (float) val;
  }

  template<>
  inline float T_to_float<uint16_t>(uint16_t val) {
    return fp16_ieee_to_fp32_value(val);
  }

  template<typename T>
  inline T float_to_T(float val) {
    return (T) val;
  }

  template<>
  inline uint16_t float_to_T<uint16_t>(float val) {
...
#pragma omp parallel for schedule(static, 65536)
  ...
  // NOTE :: good efficiency when total_vec_size is integral multiple of cache line
  ...
  inline void prefetch_vector_l2(const char *vec, size_t vecsize) {
    ...
    size_t max_prefetch_size = (vecsize / 128) * 128;
    for (size_t d = 0; d < max_prefetch_size; d += 128) {
      __builtin_prefetch((const char *) vec + d, 1, 2);
    }
#endif
  }
