  ...
  class FixedChunkPQTable {
   ...
   public:
    ...
    void load_pq_pivots_new(std::basic_istream<char> &reader, size_t num_chunks, size_t offset) {
      ...
      // Old FastDiskANN format: 4 offsets (no rearrangement)
      //   [0]=tables, [1]=centroid, [2]=chunk_offsets, [3]=file_size
      // New PipeANN format: 5 offsets
      //   [0]=tables, [1]=centroid, [2]=rearrangement, [3]=chunk_offsets, [4]=file_size
      bool use_old_format = (nr == 4);
      ...
      if (use_old_format) {
        // Old format has no rearrangement, use identity permutation
        rearrangement = new _u32[this->ndims];
        for (_u64 i = 0; i < this->ndims; i++) {
          rearrangement[i] = (_u32) i;
        }

        // chunk_offsets is at index 2 in old format
        diskann::load_bin_impl<uint32_t>(reader, chunk_offsets, nr, nc, file_offset_data[2] + offset);
      } else {
        // New format: rearrangement at index 2, chunk_offsets at index 3
      ...
      LOG(INFO) << "Loaded PQ Pivots: #ctrs: " << NUM_PQ_CENTROIDS << ", #dims: " << this->ndims
                << ", #chunks: " << this->n_chunks << ", old_format: " << use_old_format;
    }
    ...
    void populate_chunk_distances(const T *query_vec, float *dist_vec) {
      ...
      for (_u64 chunk = 0; chunk < n_chunks; chunk++) {
        ...
        for (_u64 j = chunk_offsets[chunk]; j < chunk_offsets[chunk + 1]; j++) {
          ...
          float query_val = diskann::T_to_float<T>(query_vec[permuted_dim_in_query]);
          for (_u64 idx = 0; idx < 256; idx++) {
            double diff = centers_dim_vec[idx] - (query_val - centroid[permuted_dim_in_query]);
      ...
      for (_u64 chunk = 0; chunk < n_chunks; chunk++) {
        ...
        for (_u64 j = chunk_offsets[chunk]; j < chunk_offsets[chunk + 1]; j++) {
          ...
          for (_u64 idx = 0; idx < 256; idx += 16) {
            ...
            float query_val = diskann::T_to_float<T>(query_vec[permuted_dim_in_query]);
            __m512 query_f = _mm512_set1_ps(query_val - centroid[permuted_dim_in_query]);
