...
// Read VmRSS or VmHWM from /proc/self/status, return value in MB.
static double get_proc_vm_field_mb(const char *field) {
  ...
  while (fgets(line, sizeof(line), f)) {
    if (strncmp(line, field, strlen(field)) == 0) {
      // e.g. "VmRSS:    123456 kB"
      sscanf(line + strlen(field), " %lf kB", &val);
      val /= 1024.0;  // kB -> MB
      break;
    }
  }
...
static double get_rss_mb() {
  return get_proc_vm_field_mb("VmRSS:");
}
static double get_peak_rss_mb() {
  return get_proc_vm_field_mb("VmHWM:");
}
...
int search_disk_index(int argc, char **argv) {
  ...
  _u32 repeat = (index < argc) ? std::atoi(argv[index++]) : 1;
  if (repeat == 0)
    repeat = 1;
  ...
  LOG(INFO) << "RSS before load: " << get_rss_mb() << " MB, peak: " << get_peak_rss_mb() << " MB";
  int res = _pFlashIndex->load(index_prefix_path.c_str(), num_threads, true, use_page_search);
  if (res != 0) {
    return res;
  }
  LOG(INFO) << "RSS after load: " << get_rss_mb() << " MB, peak: " << get_peak_rss_mb() << " MB";
  ...
  if (mem_L != 0) {
    ...
    LOG(INFO) << "RSS after load_mem_index: " << get_rss_mb() << " MB, peak: " << get_peak_rss_mb() << " MB";
  }
  ...
  auto run_tests = [&](uint32_t test_id, bool output) {
    ...
    memset(stats, 0, sizeof(diskann::QueryStats) * query_num);
    _u64 L = Lvec[test_id];
    ...
    for (_u32 iter = 0; iter < repeat; iter++) {
...
#pragma omp parallel for schedule(dynamic, 1)
      ...
      total_qps += (1.0 * (double) query_num) / (1.0 * (double) diff.count());
    }

    // average over repeats
    float qps = (float) (total_qps / repeat);
    ...
    if (output) {
      ...
      diskann::cout << std::setw(6) << L << std::setw(12) << beamwidth << std::setw(12) << qps << std::setw(12)
                    ...
                    << mean_ios << std::setw(10) << rss << std::setw(10) << peak_rss;
      if (calc_recall_flag) {
  ...
  diskann::cout << std::setw(6) << "L" << std::setw(12) << "I/O Width" << std::setw(12) << "QPS" << std::setw(12)
                ...
                << "Mean IOs" << std::setw(10) << "RSS(MB)" << std::setw(10) << "Peak(MB)" << std::setw(12);
  if (calc_recall_flag) {
...
int main(int argc, char **argv) {
  if (argc < 13) {
    ...
    diskann::cout << "Usage: " << argv[0]
                  << " <index_type (float/int8/uint8/float16)>  <index_prefix_path>"
                     ...
                     "using mem index)> <repeat> <L1> [L2] etc."
                  << std::endl;
    exit(-1);
  }
  ...
  else if (std::string(argv[1]) == std::string("float16"))
    search_disk_index<fp16_defined>(argc, argv);
  else
    diskann::cout << "Unsupported index type. Use float or int8 or uint8 or float16" << std::endl;
}
