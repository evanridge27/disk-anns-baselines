...
bool build_index(const char *dataFilePath, const char *indexFilePath, const char *indexBuildParameters,
                 diskann::Metric m, bool singleFile) {
  return diskann::build_disk_index<T>(dataFilePath, indexFilePath, indexBuildParameters, m, singleFile);
}

int main(int argc, char **argv) {
  if (argc != 11) {
    diskann::cout << "Usage: " << argv[0]
                  << " <data_type (float/int8/uint8/float16)>  <data_file.bin>"
    ...
    if (dist_metric != "l2" && m == diskann::Metric::L2) {
      diskann::cout << "Metric " << dist_metric << " is not supported. Using L2" << std::endl;
    }
    if (std::string(argv[1]) == std::string("float"))
      ...
      build_index<fp16_defined>(argv[2], argv[3], params.c_str(), m, single_file_index);
    else
