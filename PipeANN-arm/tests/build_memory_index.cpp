...
int build_in_memory_index(const std::string &data_path, const std::string &tags_file, const unsigned R,
                          ...
                          bool dynamic_index, bool single_file_index, diskann::Metric distMetric) {
  ...
  diskann::cout << "Building in-memory index with parameters: data_file: " << data_path << "tags file: " << tags_file
                << " R: " << R << " L: " << L << " alpha: " << alpha << " index_path: " << save_path
                << " #threads: " << num_threads
                << ", using distance metric: " << (distMetric == diskann::Metric::COSINE ? "cosine " : "l2 ");

  typedef uint32_t TagT;

  diskann::Index<T, TagT> index(distMetric, data_dim, data_num, dynamic_index, single_file_index,
  ...
  if (dynamic_index) {
    ...
    std::ifstream reader;
    diskann::cout << "Opening bin file " << tags_file << "... " << std::endl;
    ...
    reader.close();
    diskann::cout << "First tag is " << tags[0] << std::endl;
...
int main(int argc, char **argv) {
  if (argc != 12) {
    diskann::cout << "Usage: " << argv[0]
                  << " <data_type(int8/uint8/float/float16)>  <data_file.bin>"
  ...
  enum diskann::Metric distMetric = dist_metric_str == "cosine"
  ...
  if (std::string(argv[1]) == std::string("int8"))
    ...
    build_in_memory_index<fp16_defined>(data_path, tags_file, R, L, alpha, save_path, num_threads, dynamic_index,
                                        single_file_index, distMetric);
  else
    diskann::cout << "Unsupported type. Use float/int8/uint8/float16" << std::endl;
}
