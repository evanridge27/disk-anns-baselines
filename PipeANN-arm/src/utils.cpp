...
#elif defined(_ARM)
// On ARM, Avx2SupportedCPU=true means "use NEON optimized path"
...
namespace diskann {
  ...
  diskann::Distance<int8_t> *get_distance_function(diskann::Metric m) {
    if (m == diskann::Metric::L2) {
      if (Avx2SupportedCPU) {
        ...
        diskann::cout << "Older CPU. Using slow distance computation" << std::endl;
        return new diskann::SlowDistanceL2Int<int8_t>();
      }
    } else if (m == diskann::Metric::COSINE) {
      diskann::cout << "Using either AVX or AVX2 for Cosine similarity" << std::endl;
  ...
  template<>
  diskann::Distance<fp16_defined> *get_distance_function(diskann::Metric m) {
    if (m == diskann::Metric::L2) {
      diskann::cout << "L2: Using FP16 distance computation (fp16->fp32 via SIMD)" << std::endl;
      return new diskann::DistanceL2FP16();
    } else if (m == diskann::Metric::COSINE) {
      diskann::cout << "Cosine: Using FP16 cosine distance computation (fp16->fp32)" << std::endl;
      return new diskann::DistanceCosineFP16();
    } else {
      std::stringstream stream;
      stream << "Only L2 and cosine metric supported for fp16 as of now." << std::endl;
      std::cerr << stream.str() << std::endl;
      throw diskann::ANNException(stream.str(), -1, __FUNCSIG__, __FILE__, __LINE__);
    }
  }
