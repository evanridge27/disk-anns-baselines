...
#pragma omp critical
      {
        ...
        diskann::alloc_aligned((void **) &scratch.aligned_pqtable_dist_scratch, 256 * this->n_chunks * sizeof(float),
                               256);
  ...
  std::vector<_u8> PQFlashIndex<T, TagT>::deflate_vector(const T *vec) {
    ...
    for (uint32_t i = 0; i < this->data_dim; i++) {
      fp_vec[i] = diskann::T_to_float<T>(vec[i]);
    }
...
#pragma omp parallel for num_threads(max_nthreads)
  ...
  template class DiskNode<fp16_defined>;
