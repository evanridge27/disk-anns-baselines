  ...
  void PQFlashIndex<T, TagT>::disk_iterate_to_fixed_point_v(
    ...
    if (data_is_normalized) {
      ...
      for (uint32_t i = 0; i < this->data_dim; i++) {
        data.scratch.aligned_query_float[i] = diskann::T_to_float<T>(query1[i]) / norm;
      }
      for (uint32_t i = 0; i < this->data_dim; i++) {
        data.scratch.aligned_query_T[i] = diskann::float_to_T<T>(data.scratch.aligned_query_float[i]);
      }
    } else {
      for (uint32_t i = 0; i < this->data_dim; i++) {
...
#elif defined(_ARM)
    __builtin_prefetch((char *) scratch, 0, 3);
#endif
