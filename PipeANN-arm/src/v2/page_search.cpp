...
namespace diskann {
  ...
  void PQFlashIndex<T, TagT>::load_page_layout(const std::string &index_prefix, const _u64 nnodes_per_sector,
...
#pragma omp parallel for
        for (size_t i = 0; i < num_sectors; ++i) {
          ...
          for (uint32_t j = 0; j < nnodes_per_sector; ++j) {
            tmp_arr[j] = i * nnodes_per_sector + j;
          }
  ...
  template<typename T, typename TagT>
  size_t PQFlashIndex<T, TagT>::page_search(const T *query1, const _u64 k_search, const _u32 mem_L, const _u64 l_search,
    ...
    uint32_t query_dim = this->data_dim;

    for (uint32_t i = 0; i < query_dim; i++) {
      data.scratch.aligned_query_float[i] = diskann::T_to_float<T>(query1[i]);
      data.scratch.aligned_query_T[i] = query1[i];
      float qf = diskann::T_to_float<T>(query1[i]);
      query_norm += qf * qf;
    }
...
#elif defined(_ARM)
    __builtin_prefetch((char *) data_buf, 1, 2);
#endif
    ...
    while (k < cur_list_size && num_ios < IO_LIMIT) {
      ...
      if (!frontier.empty()) {
        ...
        reader->send_io(frontier_read_reqs, ctx, false);
        n_ios = frontier_read_reqs.size();
...
#pragma omp critical
  ...
  template class PQFlashIndex<fp16_defined, uint32_t>;
