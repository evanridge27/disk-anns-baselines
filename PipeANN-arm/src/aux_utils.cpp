...
namespace diskann {
  ...
  template DISKANN_DLLEXPORT float *load_warmup<float>(const std::string &cache_warmup_file, uint64_t &warmup_num,
                                                       uint64_t warmup_dim, uint64_t warmup_aligned_dim);
  template DISKANN_DLLEXPORT fp16_defined *load_warmup<fp16_defined>(const std::string &cache_warmup_file,
  ...
  template DISKANN_DLLEXPORT bool build_disk_index<float, uint32_t>(const char *dataFilePath, const char *indexFilePath,
                                                                    ...
                                                                    diskann::Metric _compareMetric,
                                                                    bool singleFileIndex, const char *tag_file);
  template DISKANN_DLLEXPORT bool build_disk_index<fp16_defined, uint32_t>(const char *dataFilePath,
                                                                           const char *indexFilePath,
                                                                           const char *indexBuildParameters,
                                                                           diskann::Metric _compareMetric,
                                                                           bool singleFileIndex, const char *tag_file);
  ...
  template DISKANN_DLLEXPORT int build_merged_vamana_index<uint8_t>(
      ...
      std::string centroids_file, const char *tag_file);
  template DISKANN_DLLEXPORT int build_merged_vamana_index<fp16_defined>(
      std::string base_file, diskann::Metric _compareMetric, bool single_file_index, unsigned L, unsigned R,
      double sampling_rate, double ram_budget, std::string mem_index_path, std::string medoids_path,
      std::string centroids_file, const char *tag_file);
};  // namespace diskann
