...
#define REPEAT(type, ctype, delta, load, exec, acc, result) \
...
#endif
#elif defined(_ARM)  // ARM platform - use OpenMP SIMD (auto-vectorized by compiler)
  ...
  float DistanceL2Int8::compare(const int8_t *a, const int8_t *b, uint32_t size) const {
    int32_t result = 0;
#pragma omp simd reduction(+ : result) aligned(a, b : 16)
...
#pragma omp simd reduction(+ : result) aligned(a, b : 16)
  ...
  float AVXDistanceL2Float::compare(const float *a, const float *b, uint32_t size) const {
    // On ARM, use OpenMP SIMD same as DistanceL2
    float result = 0;
#pragma omp simd reduction(+ : result) aligned(a, b : 16)
    for (int32_t i = 0; i < (int32_t) size; i++) {
      result += (a[i] - b[i]) * (a[i] - b[i]);
  ...
  // FP16 distance on ARM: use NEON to convert fp16 -> fp32, then compute L2 in fp32
  float DistanceL2FP16::compare(const fp16_defined *a, const fp16_defined *b, uint32_t size) const {
    uint32_t i = 0;
    float32x4_t acc0 = vdupq_n_f32(0.0f);

    const float16_t *a_fp16 = reinterpret_cast<const float16_t *>(a);
    const float16_t *b_fp16 = reinterpret_cast<const float16_t *>(b);

    for (; i + 4 <= size; i += 4) {
      float16x4_t vec1_fp16 = vld1_f16(a_fp16 + i);
      float16x4_t vec2_fp16 = vld1_f16(b_fp16 + i);
      float32x4_t vec1_f32 = vcvt_f32_f16(vec1_fp16);
      float32x4_t vec2_f32 = vcvt_f32_f16(vec2_fp16);
      float32x4_t diff = vsubq_f32(vec1_f32, vec2_f32);
    ...
    // tail processing
    for (; i < size; ++i) {
      float da = (float) a_fp16[i];
      float db = (float) b_fp16[i];
      float d = da - db;
      result += d * d;
    }
  ...
  // FP16 cosine distance on ARM
  float DistanceCosineFP16::compare(const fp16_defined *a, const fp16_defined *b, uint32_t length) const {
    return diskann::compute_cosine_similarity(a, b, length);
  }

#else  // without AVX512 support (x86 fallback)
...
#pragma omp simd reduction(+ : result) aligned(a, b : 32)
  ...
  // FP16 distance implementations for x86 (AVX2 F16C path)
#ifndef _WINDOWS
#ifdef __AVX2__
  float DistanceL2FP16::compare(const fp16_defined *a, const fp16_defined *b, uint32_t size) const {
    const uint16_t *pa = a;
    ...
    uint32_t niters = size / 8;
    for (uint32_t j = 0; j < niters; ++j, i += 8) {
      __m128i a_half = _mm_loadu_si128(reinterpret_cast<const __m128i *>(pa + i));
      __m128i b_half = _mm_loadu_si128(reinterpret_cast<const __m128i *>(pb + i));
      __m256 a_f32 = _mm256_cvtph_ps(a_half);
      __m256 b_f32 = _mm256_cvtph_ps(b_half);
    ...
    for (; i < size; ++i) {
      float fa = fp16_ieee_to_fp32_value(pa[i]);
      float fb = fp16_ieee_to_fp32_value(pb[i]);
      float d = fa - fb;
      result += d * d;
    }
...
#elif !defined(_ARM)
  // Non-AVX2 x86: scalar fallback for FP16
  float DistanceL2FP16::compare(const fp16_defined *a, const fp16_defined *b, uint32_t size) const {
    const uint16_t *pa = a;
    const uint16_t *pb = b;
    float result = 0;
