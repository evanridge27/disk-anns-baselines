...
#pragma omp critical
  ...
  unsigned Index<T, TagT>::calculate_entry_point() {
    ...
    for (size_t i = 0; i < _nd; i++)
      for (size_t j = 0; j < _aligned_dim; j++)
        center[j] += diskann::T_to_float<T>(_data[i * _aligned_dim + j]);
...
#pragma omp parallel for schedule(static, 65536)
    for (_s64 i = 0; i < (_s64) _nd; i++) {
      ...
      float diff = 0;
      for (size_t j = 0; j < _aligned_dim; j++) {
        diff = (center[j] - diskann::T_to_float<T>(cur_vec[j])) * (center[j] - diskann::T_to_float<T>(cur_vec[j]));
        dist += diff;
      }
    }
    // find imin
...
#pragma omp parallel for schedule(dynamic)
  ...
  template DISKANN_DLLEXPORT class Index<fp16_defined, uint32_t>;
